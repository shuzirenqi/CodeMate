...
- v0.5.0：ReAct、Plan-and-Execute、Memory、RAG、SSE 流式输出、终端渲染与文件日志；Java 17、Maven。
- 入口：`com.codemate.cli.Main`；产物：`target/codemate-0.5.0.jar`。
...
- 当前包含 CLI、Plan、Memory、RAG 和终端渲染测试；不得将单元测试通过描述成真实模型或 Embedding 联调通过。
