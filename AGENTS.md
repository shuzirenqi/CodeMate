...
- v0.8.0：ReAct、Plan-and-Execute、Multi-Agent、Memory、RAG、HITL 审批、异步并行工具调用、SSE 流式输出、终端渲染与文件日志；Java 17、Maven。
- 入口：`com.codemate.cli.Main`；产物：`target/codemate-0.8.0.jar`。
...
- 当前包含 CLI、Plan、Multi-Agent、Memory、RAG、HITL、并行工具、流式渲染和终端渲染测试；不得将单元测试通过描述成真实模型或 Embedding 联调通过。
