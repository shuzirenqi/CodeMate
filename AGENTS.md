...
- v0.4.0：ReAct、Plan-and-Execute、DAG、Memory、RAG 与 6 个工具；Java 17、Maven。
- 入口：`com.codemate.cli.Main`；产物：`target/codemate-0.4.0.jar`。
...
- 当前包含 CLI、Plan、Memory 和 RAG 测试；不得将单元测试通过描述成真实模型或 Embedding 联调通过。
