...
import com.codemate.rag.CodeRetriever;
import com.codemate.rag.SearchResultFormatter;
import com.codemate.rag.VectorStore;
import com.codemate.web.FetchResult;
import com.codemate.web.HtmlExtractor;
import com.codemate.web.NetworkPolicy;
import com.codemate.web.SearchProvider;
import com.codemate.web.SearchProviderFactory;
import com.codemate.web.SearchResult;
import com.codemate.web.WebFetcher;
...
public class ToolRegistry {
    ...
    private void registerWebTools() {
        tools.put("web_search", new Tool(
                ...
                "搜索互联网，获取实时信息（最新版本、官方文档、技术资讯等）。" +
                        "支持智谱、SerpAPI 和 SearXNG provider，可由 SEARCH_PROVIDER 环境变量切换。",
                createParameters(
     ...
     * 获取所有工具定义（用于LLM）
     */
    public List<com.codemate.llm.LlmClient.Tool> getToolDefinitions() {
        return tools.values().stream()
                .map(t -> new com.codemate.llm.LlmClient.Tool(t.name(), t.description(), t.parameters()))
                .toList();
    }
    ...
    public List<ToolExecutionResult> executeTools(List<ToolInvocation> invocations) {
        ...
        ExecutorService executor = Executors.newFixedThreadPool(parallelism, r -> {
            Thread thread = new Thread(r, "codemate-tool-executor");
    ...
    private String executeCommand(String command) {
        ...
        // Git Bash 的 pwd 会返回 MSYS 路径。直接使用 Java 路径可在 Windows 上稳定
        // 返回项目目录，并避免短生命周期 shell 持有测试临时目录句柄。
        if ("pwd".equals(normalized)) {
            String workingDirectory = Path.of(projectPath).toAbsolutePath().normalize().toString();
            return "命令执行完成 (exit code: 0)\n工作目录: " + workingDirectory + "\n" + workingDirectory + "\n";
        }

        ExecutorService outputReaderExecutor = Executors.newSingleThreadExecutor(r -> {
            Thread thread = new Thread(r, "codemate-command-output");
            thread.setDaemon(true);
            return thread;
        });
        ...
        try {
            ProcessBuilder pb = new ProcessBuilder(resolveBashExecutable(), "-c", normalized);
            ...
            return String.format("命令执行完成 (exit code: %d)\n工作目录: %s\n%s",
                    exitCode, workingDirectory, output);
        } catch (Exception e) {
            if (process != null) {
                terminateProcessTree(process);
            }
            ...
            try {
                outputReaderExecutor.awaitTermination(2, TimeUnit.SECONDS);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }
    ...
    /**
     * Windows 自带的 System32/bash.exe 是 WSL 启动器，WSL 未启用时会直接失败。
     * CodeMate 的 Shell 语义按 Git Bash 设计，因此 Windows 优先定位 Git Bash；
     * 其他安装位置可通过 -Dcodemate.shell.bash=... 显式指定。
     */
    private String resolveBashExecutable() {
        String configured = System.getProperty("codemate.shell.bash");
        ...
        addBashCandidate(candidates, System.getenv("ProgramFiles"), "Git", "bin", "bash.exe");
        addBashCandidate(candidates, System.getenv("ProgramFiles(x86)"), "Git", "bin", "bash.exe");
        addBashCandidate(candidates, System.getenv("LOCALAPPDATA"), "Programs", "Git", "bin", "bash.exe");
    ...
    private void addBashCandidate(List<Path> candidates, String root, String... segments) {
    ...
    /**
     * 终止 Shell 以及它启动的子进程，防止子进程继续占用项目目录或输出流。
     */
    private void terminateProcessTree(Process process) {
        List<ProcessHandle> handles = new ArrayList<>(process.toHandle().descendants().toList());
        ...
        while (System.nanoTime() < deadline) {
            boolean anyAlive = process.isAlive() || handles.stream().anyMatch(ProcessHandle::isAlive);
            if (!anyAlive) {
                return;
            }
            try {
