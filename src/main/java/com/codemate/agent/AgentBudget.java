 ...
 * 1. 系统属性：{@code codemate.react.token.budget} / {@code codemate.react.stagnation.window} /
 *    {@code codemate.react.hard.max.iterations}
...
public class AgentBudget {
    ...
    public static AgentBudget fromSystemProperties() {
        return new AgentBudget(
                readIntProperty("codemate.react.token.budget", DEFAULT_TOKEN_BUDGET),
                readIntProperty("codemate.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("codemate.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
